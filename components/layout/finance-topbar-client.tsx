...
import { FinanceWorkspaceBanner } from "@/components/layout/finance-workspace-banner";

export function FinanceTopbarClient(props: {
  workspace: FinanceWorkspace | null;
  sessionEmail: string;
}) {
  const { workspace, sessionEmail } = props;
  ...
  return (
    <>
      ...
      <header className="flex flex-col flex-shrink-0 border-b border-white/8 bg-[#0d0d0d]/95 backdrop-blur-md supports-[backdrop-filter]:bg-[#0d0d0d]/80">
        <div className="flex items-center justify-between h-14 px-4 sm:px-6 gap-3 flex-shrink-0">
          <div className="flex items-center gap-3 min-w-0 flex-shrink">
            ...
            {workspace ? <FinanceWorkspaceBanner workspace={workspace} /> : null}
          </div>
          {/* Search */}
          ...
          <div className="flex items-center gap-2 flex-shrink-0">
            ...
            <div className="text-xs text-neutral-300 truncate max-w-[10rem] sm:max-w-[14rem] rounded-md border border-white/10 px-2 py-1.5 bg-white/[0.03] hidden sm:block">
